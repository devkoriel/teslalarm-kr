...
async def analyze_and_extract_fields(consolidated_text: str, language: str = "ko") -> dict:
    ...
    prompt = (
        ...
        "24. 테슬라 유용한 정보: 'useful_info_details', 'published'\n\n"
        "응답은 JSON parser로 바로 parsing 할 수 있도록 반드시 아래 JSON 형식으로 해 (미리 정의된 카테고리에 속하지 않는 뉴스나 정보성 글은 포함하지마):\n"
        "{\n"
        ...
        '  "useful_info": [ { "title": "...", "useful_info_details": "...", "published": "...", "urls": ["...", ...] }, ... ],\n'
        "}\n\n"
        ...
        "테슬라 구매 보조금 정보와 테슬라 유용한 정보는 뉴스가 아닌 정보성 글이야. "
        ...
        "테슬라 유용한 정보는 구매와 할인, 할부, 보조금, 유용한 제품 & 용품, 후기, 유용한 정보 같이 테슬라 차주들이 관심있을만한 유용한 내용만 남기고 광고나 홍보, 주식 관련 글은 무조건 걸러. 특히 정보글이 단순 질문글이거나 내용에 유용한 정보가 없으면 그것도 걸러."
        "제목 앞에 [테슬라 정보]가 붙은 정보성 글은 24번 테슬라 유용한 정보로만 분류해.\n\n"
        "언어는 {language}으로 작성해.\n\n"
        "기사나 정보성 글 텍스트:\n"
    )
